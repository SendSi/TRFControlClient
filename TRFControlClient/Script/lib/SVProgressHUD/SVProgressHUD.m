...
+(void)showTitle:(NSString *)status timeDur:(CGFloat)timeDur{
    [[self sharedView] showImage:nil status:status duration:timeDur];
}
+(void)showTitle:(NSString *)status timeDur:(CGFloat)timeDur complete:(void(^)())complete{
    [[self sharedView] showImage:nil status:status duration:timeDur];
    complete();
}
